...
| maker | `30%` | 用来激励做市商或者普通用户挂单提供流动性 |
| creator | `2%` | 用来帮助激活交易用户的创建账户 |
| 团队 | `20%`: 前3月, `43%`: 其余 | 提供合约和系统开发的技术团队 |
| `TYCHE` | `5%` | 用于回购燃烧`TycheFi`发行的天赐币 |


- `10` 手续费返回给设置邀请人的被吃单用户 （仅限首月）；
- 
- `30%` 分配给交易的对手盘挂单者：用来鼓励挂单提供良好流动性，需要对手挂单者主动申领;
- `2%` 分配给账户的推荐人（即链上账户创建人`creator`）：需要推荐人主动申领，如手续费最新更新时间后的2周内必须申领走否则视为放弃而归属团队；

- `20%` 分配给团队用于持续技术研发和系统支持；
